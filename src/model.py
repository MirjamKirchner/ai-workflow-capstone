...
from src.part1_data_investigation.cslib import fetch_ts, engineer_features
...
def _model_train(df,tag,test=False):
    ...
    grid.fit(X, y)
    model_name = re.sub("../solution-guidance", "_", str(MODEL_VERSION))
    if test:
...
def model_load(prefix='sl',data_dir=None,training=True):
    ...
    models = [f for f in os.listdir(os.path.join("../solution-guidance", "models")) if re.search("sl", f)]
    ...
    all_models = {}
    for model in models:
        all_models[re.split("-",model)[1]] = joblib.load(os.path.join("../solution-guidance", "models", model))
